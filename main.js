...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'anima-guitar-course'; 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const adminPanelContent = adminPanel ? adminPanel.querySelector('.admin-panel-content') : null;
    ...
    const modulesSelectForLesson = document.getElementById('modulesSelectForLesson');
    ...
    const closeEditModuleModalBtn = document.getElementById('closeEditModuleModalBtn');
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) { 
            ...
            if (user.email === ADMIN_EMAIL) {
                ...
                if(adminUserIdDisplay) adminUserIdDisplay.textContent = `Admin User ID: ${currentUserId}`;
            ...
            if(modulesList) modulesList.innerHTML = '<p class="p-4 text-gray-500">Faça login para ver os módulos.</p>';
            if(lessonsContainer) lessonsContainer.innerHTML = '<p class="p-4 text-gray-500">Faça login para ver as aulas.</p>';
            if(currentModuleTitle) currentModuleTitle.textContent = 'Nenhum módulo selecionado';
            if(currentLessonTitle) currentLessonTitle.textContent = 'Nenhuma aula selecionada';
            if(videoPlayerContainer) videoPlayerContainer.innerHTML = '<p class="text-gray-500">Player de vídeo aparecerá aqui.</p>';
    ...
    if(userSignupForm) {
        userSignupForm.addEventListener('submit', async (e) => {
            ...
            const confirmPassword = userSignupForm.confirmPassword.value;
            if(userSignupError) userSignupError.textContent = '';
            if (password !== confirmPassword) {
                if(userSignupError) userSignupError.textContent = "As senhas não coincidem.";
                return;
            }
            if (password.length < 6) {
                if(userSignupError) userSignupError.textContent = "A senha deve ter pelo menos 6 caracteres.";
                return;
            }
            try {
                ...
                if(userSignupError) userSignupError.textContent = "Falha no cadastro: " + error.message;
            }
        });
    }
    ...
    if(userLoginForm) {
        userLoginForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if(userLoginError) userLoginError.textContent = "Falha no login: " + error.message;
            }
        });
    }
    
    if(pageAdminLoginBtn) pageAdminLoginBtn.addEventListener('click', () => { if(adminLoginModal) adminLoginModal.style.display = 'block'; });
    ...
    if(closeEditModuleModalBtn) closeEditModuleModalBtn.addEventListener('click', () => { if(editModuleModal) editModuleModal.style.display = 'none'; });
    if(closeEditLessonModalBtn) closeEditLessonModalBtn.addEventListener('click', () => { if(editLessonModal) editLessonModal.style.display = 'none'; });

    window.onclick = function(event) { 
        if (adminLoginModal && event.target == adminLoginModal) adminLoginModal.style.display = "none";
        if (editModuleModal && event.target == editModuleModal) editModuleModal.style.display = "none";
        if (editLessonModal && event.target == editLessonModal) editLessonModal.style.display = "none";
    }
    ...
    if(adminLoginForm) {
        adminLoginForm.addEventListener('submit', async (e) => {
            ...
            if (email !== ADMIN_EMAIL) { 
                if(adminLoginError) adminLoginError.textContent = "Este não é o email do administrador.";
                return;
            }
            try {
                await signInWithEmailAndPassword(auth, email, password);
                ...
                if(adminLoginError) adminLoginError.textContent = "Falha no login do admin: " + error.message;
            }
        });
    }
    ...
    window.addEventListener('resize', () => {
        if (window.innerWidth >= 768 && sidebar && !sidebar.classList.contains('-translate-x-full') && mainContent) {
            mainContent.style.marginLeft = sidebar.offsetWidth + 'px';
        } else if (mainContent) {
            mainContent.style.marginLeft = '0';
        }
    });
     if (window.innerWidth >= 768 && openSidebarButton && sidebar && !sidebar.classList.contains('-translate-x-full')) { 
    }
    ...
    async function loadModules() {
        ...
        if (!modulesList) { console.error("modulesList element not found."); return; }

        modulesList.innerHTML = '<p class="p-4 text-gray-500">Carregando módulos...</p>';
        const modulesCol = collection(db, modulesCollectionPath);
        
        try {
            if (window.unsubscribeModules) window.unsubscribeModules();

            window.unsubscribeModules = onSnapshot(modulesCol, (querySnapshot) => {
                const modules = [];
                ...
                if (modulesSelectForLesson) modulesSelectForLesson.innerHTML = '<option value="">Selecione um Módulo</option>';

                if (modules.length === 0) {
                    modulesList.innerHTML = '<p class="p-4 text-gray-500">Nenhum módulo encontrado.</p>';
                }
                ...
                modules.forEach(module => {
                    ...
                    button.className = 'w-full text-left px-4 py-3 anima-blue hover:bg-blue-100 rounded-md transition-colors duration-150 flex justify-between items-center';
                    ...
                    if (isAdmin) {
                        ...
                        deleteBtn.innerHTML = '×';
                        deleteBtn.className = 'ml-2 text-red-500 hover:text-red-700 font-bold px-2';
                        deleteBtn.title = `Remover módulo ${module.name}`;
                        deleteBtn.onclick = (e) => {
                            e.stopPropagation(); 
                            if (confirm(`Tem certeza que deseja remover o módulo "${module.name}" e todas as suas aulas?`)) {
                ...
                if (modules.length > 0 && lessonsContainer && (!lessonsContainer.innerHTML.trim() || lessonsContainer.querySelector('p.text-gray-500'))) {
                   currentOpenModuleId = modules[0].id; 
                   loadLessons(modules[0].id, modules[0].name);
                }
            }, (error) => {
                console.error("Error loading modules: ", error);
                if(modulesList) modulesList.innerHTML = `<p class="p-4 text-red-500">Erro ao carregar módulos: ${error.message}</p>`;
            });
        } catch (error) {
             console.error("Error setting up modules listener: ", error);
             if(modulesList) modulesList.innerHTML = `<p class="p-4 text-red-500">Erro crítico ao configurar módulos: ${error.message}</p>`;
        }
    }

    async function loadLessons(moduleId, moduleName) {
        currentOpenModuleId = moduleId; 
        if (!auth.currentUser) { console.log("Auth not ready for loading lessons."); return; }
        if (!lessonsContainer || !currentModuleTitle || !videoPlayerContainer || !currentLessonTitle || !attachmentLink) {
            console.error("One or more lesson display elements not found."); return;
        }

        currentModuleTitle.textContent = moduleName;
        lessonsContainer.innerHTML = '<p class="p-4 text-gray-500">Carregando aulas...</p>';
        ...
        try {
            ...
            window.unsubscribeLessons = onSnapshot(lessonsCol, (querySnapshot) => {
                const lessons = [];
                querySnapshot.forEach((doc) => {
                ...
                if (lessons.length === 0) {
                    lessonsContainer.innerHTML = '<p class="p-4 text-gray-500">Nenhuma aula neste módulo.</p>';
                    return;
                }

                lessons.forEach(lesson => {
                    const div = document.createElement('div');
                    div.className = 'p-4 mb-3 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-150 flex justify-between items-center';
                    ...
                    playBtn.className = 'px-4 py-2 anima-bg-blue text-white rounded-md hover:bg-blue-700 transition-colors duration-150 text-sm';
                    ...
                    if (isAdmin) {
                        ...
                        editBtn.onclick = (e) => {
                            e.stopPropagation();
                            openEditLessonModal(lesson.moduleId, lesson.id, lesson.title, lesson.youtubeVideoId, lesson.attachmentUrl || '', lesson.order || 0);
                        };
                        controlsDiv.appendChild(editBtn);

                        const deleteBtn = document.createElement('button');
                        deleteBtn.innerHTML = '×';
                        deleteBtn.className = 'ml-3 text-red-500 hover:text-red-700 font-bold px-2 py-1 text-sm';
                        deleteBtn.title = `Remover aula ${lesson.title}`;
                        deleteBtn.onclick = (e) => {
                            e.stopPropagation();
                            if (confirm(`Tem certeza que deseja remover a aula "${lesson.title}"?`)) {
                ...
                if(lessonsContainer) lessonsContainer.innerHTML = `<p class="p-4 text-red-500">Erro ao carregar aulas: ${error.message}</p>`;
            });
        } catch (error) {
            console.error(`Error setting up lessons listener for module ${moduleId}: `, error);
            if(lessonsContainer) lessonsContainer.innerHTML = `<p class="p-4 text-red-500">Erro crítico ao configurar aulas: ${error.message}</p>`;
        }
    }
    
    function displayLesson(lesson) {
        if (!currentLessonTitle || !videoPlayerContainer || !attachmentLink) { 
            console.error("displayLesson: Elementos de UI para exibir a aula estão ausentes.");
            return; 
        }
        ...
        console.log(
            ...
            "\n1. ID Original (Firestore):", lesson.youtubeVideoId, 
            "\n2. ID Processado (tentativa de extração):", processedVideoId, 
            "\n3. URL de Incorporação (usada no iframe):", finalEmbedUrl,
            "\n****************************************"
        );

        if (processedVideoId) { 
            console.log("displayLesson: processedVideoId é VÁLIDO. Tentando criar iframe com URL:", finalEmbedUrl);
            videoPlayerContainer.innerHTML = `
                <div class="video-container">
                    <iframe src="${finalEmbedUrl}" 
                            title="Player de vídeo do YouTube para ${lesson.title || 'aula'}"
                            frameborder="0" 
            ...
            console.log("displayLesson: processedVideoId é NULO ou INVÁLIDO. Mostrando mensagem de ID não fornecido.");
            if(videoPlayerContainer) videoPlayerContainer.innerHTML = '<p class="text-gray-500 p-4">ID do vídeo não fornecido, inválido ou não foi possível processar.</p>';
        }
    ...
    if (editModuleForm) {
        editModuleForm.addEventListener('submit', async (e) => {
            ...
            if (!newName) {
                if(editModuleError) editModuleError.textContent = "Nome do módulo é obrigatório.";
                return;
            }
            if (isNaN(newOrder)) {
                 if(editModuleError) editModuleError.textContent = "Ordem do módulo deve ser um número.";
                return;
            }
            ...
            try {
                ...
                if(editModuleError) editModuleError.textContent = "Erro ao atualizar módulo: " + error.message;
            }
        });
    }

    // --- Edit Lesson Functions ---
    function openEditLessonModal(moduleId, lessonId, title, youtubeId, attachmentUrl, order) {
    ...
    if (editLessonForm) {
        editLessonForm.addEventListener('submit', async (e) => {
            ...
            if (!newTitle || !newYoutubeId) {
                 if(editLessonError) editLessonError.textContent = "Título e ID/URL do YouTube são obrigatórios.";
                return;
            }
             if (isNaN(newOrder)) {
                 if(editLessonError) editLessonError.textContent = "Ordem da aula deve ser um número.";
                return;
            }
            ...
            try {
                ...
                console.error("Error updating lesson: ", error);
                 if(editLessonError) editLessonError.textContent = "Erro ao atualizar aula: " + error.message;
            }
        });
    }
    ...
    if(addModuleForm) {
        addModuleForm.addEventListener('submit', async (e) => {
            ...
            if (!moduleName) { alert("Nome do módulo é obrigatório."); return; }
            try {
                await addDoc(collection(db, modulesCollectionPath), {
                    name: moduleName, order: moduleOrder, createdAt: serverTimestamp() 
                });
                addModuleForm.reset();
            } catch (error) { console.error("Error adding module: ", error); alert("Erro ao adicionar módulo: " + error.message); }
        });
    }
    ...
    if(addLessonForm) {
        addLessonForm.addEventListener('submit', async (e) => {
            ...
            if (!moduleId || !lessonTitle || !youtubeId) { alert("Módulo, Título e ID/URL do YouTube são obrigatórios."); return; }
            const lessonsColPath = `${modulesCollectionPath}/${moduleId}/lessons`;
            try {
                await addDoc(collection(db, lessonsColPath), {
                    title: lessonTitle, youtubeVideoId: youtubeId, attachmentUrl: attachment, order: lessonOrder, createdAt: serverTimestamp() 
                });
                addLessonForm.reset();
                if (moduleId === currentOpenModuleId) {
                    const selectedModuleOption = Array.from(modulesSelectForLesson.options).find(opt => opt.value === moduleId);
                    if (selectedModuleOption) {
                         loadLessons(moduleId, selectedModuleOption.textContent);
                    }
                }
            } catch (error) { console.error("Error adding lesson: ", error); alert("Erro ao adicionar aula: " + error.message); }
        });
    }
    ...
    async function deleteModule(moduleId) {
        ...
        try {
            ...
            lessonsSnapshot.forEach(docRef => deletePromises.push(deleteDoc(docRef.ref)));
            ...
            if(currentLessonTitle) currentLessonTitle.textContent = 'Selecione uma aula';
            if(attachmentLink) attachmentLink.classList.add('hidden');
            currentOpenModuleId = null; 
        } catch (error) { console.error("Error deleting module: ", error); alert("Erro ao remover módulo: " + error.message); }
    }
    ...
    async function deleteLesson(moduleId, lessonId) {
        ...
        try {
            ...
            if (moduleId === currentOpenModuleId) {
                ...
                if (moduleName && moduleName !== 'Módulo Removido' && moduleName !== 'Selecione um Módulo') {
                    loadLessons(moduleId, moduleName);
                }
            }
            if (videoPlayerContainer && videoPlayerContainer.querySelector(`iframe[src*="${lessonId}"]`)) { 
                videoPlayerContainer.innerHTML = '<p class="text-gray-500 p-4">Aula removida ou vídeo não disponível.</p>';
                if(currentLessonTitle) currentLessonTitle.textContent = 'Aula Removida';
                if(attachmentLink) attachmentLink.classList.add('hidden');
            }
        } catch (error) { console.error("Error deleting lesson: ", error); alert("Erro ao remover aula: " + error.message); }
    }
});
