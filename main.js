...
document.addEventListener("DOMContentLoaded", () => {
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      if (loggedInUserEmailDisplay)
        loggedInUserEmailDisplay.textContent = user.email;
  ...
  if (userSignupForm) {
    userSignupForm.addEventListener("submit", async (e) => {
      ...
      if (password !== confirmPassword) {
        if (userSignupError)
          userSignupError.textContent = "As senhas não coincidem.";
        return;
      }
      ...
      try {
        ...
        if (userSignupError)
          userSignupError.textContent = "Falha no cadastro: " + error.message;
      }
    });
  }
  ...
  async function loadLessons(moduleId, moduleName) {
    ...
    try {
      ...
      window.unsubscribeLessons = onSnapshot(
        ...
        (error) => {
          console.error(
            `Error loading lessons for module ${moduleId}: `,
            error
          );
          if (lessonsContainer)
  ...
  async function deleteModule(moduleId) {
    ...
    try {
      ...
      if (currentModuleTitle)
        currentModuleTitle.textContent = "Módulo Removido";
  ...
  async function deleteLesson(moduleId, lessonId) {
    ...
    try {
      ...
      if (
        ...
        if (currentLessonTitle)
          currentLessonTitle.textContent = "Aula Removida";
        if (attachmentLink) attachmentLink.classList.add("hidden");
      }
    } catch (error) {
